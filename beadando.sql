...
SELECT UGYFEL_FK AS 'ÜGYFÉL',COUNT(*) AS 'FOGLALÁSOK SZÁMA'
FROM Foglalas
WHERE UGYFEL_FK IS NOT NULL
GROUP BY UGYFEL_FK
ORDER BY COUNT(*) DESC
...
SELECT SZOBA_FK AS 'SZOBA', SUM(DATEDIFF(DAY,METTOL,MEDDIG)) AS 'FOGLALT NAPOK SZÁMA'
FROM Foglalas
GROUP BY SZOBA_FK
ORDER BY SZOBA ASC
...
SELECT DATENAME(WEEKDAY,METTOL) AS 'NAP', COUNT(*) AS 'FOGLALÁSOK SZÁMA'
FROM Foglalas
WHERE MEDDIG IS NOT NULL
GROUP BY DATENAME(WEEKDAY,METTOL)
ORDER BY DATENAME(WEEKDAY,METTOL)
